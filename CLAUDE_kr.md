...
- `config.yaml`은 대표적인 런타임 설정 예시이며, cleanup 스케줄러, 주기적 Redis 기록, snapshot HTTP API를 함께 표현할 수 있는 기준 설정입니다.
...
현재 테스트는 `actor/Go2RTCActor_test.go`, `actor/RedisActor_test.go`, `actor/SnapshotActor_test.go`, `actor/StreamCountActor_test.go`, `config/config_test.go`, `httpserver/server_test.go`에 있습니다.
...
- `config` 테스트는 `app.box_ip`, cleanup cron 검증, Redis publish interval 검증을 다룹니다.
- `httpserver` 테스트는 snapshot HTTP 핸들러 응답 형태를 검증합니다.
